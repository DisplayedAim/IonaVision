        ...
        while True:
            ...
            if self.autoExpose:
                if(self.autoExpose != self.prevValue):
                    ...
                    self.webcam.setWhiteBalanceManual(3633)
            else:
                if (self.autoExpose != self.prevValue):
                    self.prevValue = self.autoExpose
                    self.webcam.setExposureManual(0)
                    self.webcam.setBrightness(0)
                    self.webcam.setWhiteBalanceManual(10000)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        webcam = cameras[networkTable.getDouble("WhichCamera", 0)]
        cameraServer = streams[networkTable.getDouble("WhichCamera", 0)]
        #Sets video reader to read from new webcam and cameraServer
        cap.webcam = webcam
        cap.cameraServer = cameraServer
        cap.stream = cameraServer.getVideo()
        #Set video streamer to stream from new cameraServer
        streamViewer.outputStream = cameraServer.putVideo("stream", image_width, image_height)

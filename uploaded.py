...
text = False
print('Type yes for Vision Targets or anything else for Hatch panels:')
hello = input()
if hello == True:
    text = True
else:
...
def findTargets(contours, image, centerX, centerY):
    ...
    if (len(targets) > 0):
        ...
        #Trying to figure out how to send stuff to the SmartDashboard
...
############################################################################################################
#Hatch panel code.
def threshold_video2(frame):
    blur = cv2.medianBlur(frame, 5)    
    hsv = cv2.cvtColor(blur, cv2.COLOR_BGR2HSV)
    lower_color = np.array([23,137,178])
    upper_color = np.array([40,255,255])
    mask = cv2.inRange(hsv, lower_color, upper_color)

    return mask

def findContours2(frame, mask):
    _, contours, _ = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_TC89_KCOS)
    screenHeight, screenWidth, _ = frame.shape
    ...
    if len(contours) != 0:
        image = findTargets2(contours, image, centerX, centerY)
        
    return image

def findTargets2(contours, image, centerX, centerY):
    screenHeight, screenWidth, channels = image.shape;
    targets = []
...
if __name__ == "__main__":
    ...
    while True:
        ...
        #Vision Targets Run
         # Text is input that will be typed as a question, if yes is inputted then the vision target program will run
    if text == True: 
        threshold = threshold_video(frame)
        processed = findContours(frame, threshold)
        outputStream.putFrame(processed)
    else:
        print("brightness",cameras[0].getBrightness())
        cameras[0].setBrightness(50);
        cameras[0].setExposureManual(50);
        cameras[0].setWhiteBalanceAuto(10000);
        threshold2= threshold_video2(frame);
        processed2 = findContours2(frame, threshold2)
        outputStream.putFrame(processed2)
